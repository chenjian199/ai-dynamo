...
def _process_result_file(result_files: Dict, json_file: str, concurrency: int, isl: float, osl: float, 
                         deployment_name: str, filter_isl: float, filter_osl: float, filter_deployment: str) -> None:
    """
    处理单个结果文件，应用过滤器并添加到结果字典中
    """
    # 如果指定了过滤器，检查是否符合
...
def find_result_files(base_dir: str = None, mode: str = "agg", filter_isl: float = None, filter_osl: float = None, filter_deployment: str = None) -> Dict[tuple, tuple]:
    ...
    if base_dir is None:
        ...
        base_dir = str(cjworkspace_dir / "temp/sglangv2/disagg")
    else:
        # 如果指定了base_dir，确保它是绝对路径（相对于当前工作目录）
        if not os.path.isabs(base_dir):
            # 转换为绝对路径（相对于当前工作目录，通常是项目根目录）
            base_dir = os.path.abspath(base_dir)
    
    # 根据模式选择目录模式
    # 支持两种格式：
    # 1. 新格式（嵌套结构）: {mode}_{deployment_name}_isl{isl}_osl{osl}/c{concurrency}/
    # 2. 旧格式（扁平结构）: {mode}_{deployment_name}_isl{isl}_osl{osl}_concurrency{concurrency}/
    # 3. 旧格式（无部署名）: {mode}_isl{isl}_osl{osl}_concurrency{concurrency}/
    
    # 首先尝试新格式：查找 {mode}*_isl*_osl* 目录（不包含 concurrency）
    if mode == "disagg":
        parent_pattern = os.path.join(base_dir, "disagg*_isl*_osl*")
    else:
        parent_pattern = os.path.join(base_dir, "agg*_isl*_osl*")
    
    parent_dirs = glob.glob(parent_pattern)
    
    # 也尝试旧格式：查找包含 concurrency 的目录
    if mode == "disagg":
        old_pattern = os.path.join(base_dir, "disagg*_isl*_osl*_concurrency*")
    else:
        old_pattern = os.path.join(base_dir, "agg*_isl*_osl*_concurrency*")
    
    old_dirs = glob.glob(old_pattern)
    
    # 合并所有目录，去重
    all_dirs = list(set(parent_dirs + old_dirs))
    
    for test_dir in all_dirs:
        # 从目录名提取信息
        dir_name = os.path.basename(test_dir)
        
        # 尝试匹配新格式: {mode}_{deployment_name}_isl{isl}_osl{osl}（不包含 concurrency）
        # 支持两种格式：agg4a_isl... 和 agg_4a_isl...（下划线可选）
        match = re.match(rf'{mode}(?:_)?(.+?)_isl(\d+(?:\.\d+)?)_osl(\d+(?:\.\d+)?)$', dir_name)
        ...
        if match:
            # 新格式：父目录包含部署名、ISL、OSL，并发度在子目录 c* 中
            deployment_name = match.group(1)
            isl = float(match.group(2))
            osl = float(match.group(3))
            
            # 在父目录下查找 c* 子目录
            concurrency_dirs = glob.glob(os.path.join(test_dir, "c*"))
            for concurrency_dir in concurrency_dirs:
                concurrency_dir_name = os.path.basename(concurrency_dir)
                # 提取并发度：c1 -> 1, c10 -> 10, c100 -> 100
                concurrency_match = re.match(r'^c(\d+)$', concurrency_dir_name)
                if concurrency_match:
                    concurrency = int(concurrency_match.group(1))
                    # 在这个并发度目录下查找 JSON 文件（支持多种文件名格式）
                    json_patterns = [
                        os.path.join(concurrency_dir, "**", "profile_export_genai_perf.json"),
                        os.path.join(concurrency_dir, "**", "profile_export_aiperf.json"),
                        os.path.join(concurrency_dir, "profile_export_genai_perf.json"),
                        os.path.join(concurrency_dir, "profile_export_aiperf.json"),
                    ]
                    json_files = []
                    ...
                    if json_files:
                        ...
                        # 处理这个并发度的结果
                        _process_result_file(result_files, json_file, concurrency, isl, osl, deployment_name, filter_isl, filter_osl, filter_deployment)
        else:
            # 尝试匹配旧格式: {mode}_{deployment_name}_isl{isl}_osl{osl}_concurrency{concurrency}
            # 支持两种格式：agg4a_isl... 和 agg_4a_isl...（下划线可选）
            ...
            if match:
                ...
                # 尝试匹配旧格式（无部署名）: {mode}_isl{isl}_osl{osl}_concurrency{concurrency}
                ...
                if match:
                    ...
                    # 如果无法解析，尝试从JSON文件读取（支持多种文件名格式）
                    json_patterns = [
                        os.path.join(test_dir, "**", "profile_export_genai_perf.json"),
                        os.path.join(test_dir, "**", "profile_export_aiperf.json"),
                        os.path.join(test_dir, "profile_export_genai_perf.json"),
                        os.path.join(test_dir, "profile_export_aiperf.json"),
                    ]
                    json_files = []
            ...
            # 对于旧格式，直接在目录下查找 JSON 文件（支持多种文件名格式）
...
def main():
    ...
    parser.add_argument(
        '--output-dir',
        type=str,
        default="cjworkspace/results/sglangv2",
    ...
    if not result_files:
        ...
        if args.mode == "disagg":
            ...
            print(f"     - {search_dir}/disagg*_isl*_osl*/c*/profile_export_*.json (新格式：嵌套结构)")
            print(f"     - {search_dir}/disagg*_isl*_osl*_concurrency*/profile_export_*.json (旧格式：扁平结构)")
        else:
            print(f"   Searched patterns:")
            print(f"     - {search_dir}/agg*_isl*_osl*/c*/profile_export_*.json (新格式：嵌套结构)")
            print(f"     - {search_dir}/agg*_isl*_osl*_concurrency*/profile_export_*.json (旧格式：扁平结构)")
        sys.exit(1)
