#!/usr/bin/env python3
"""
计算相同并发度下，两个部署的 ITL p90 倍数关系

x轴: 并发度 (concurrency)
y轴: ITL p90 倍数 (disagg / agg)
"""
...
def calculate_ratio(
    ...
    metric_col: str = 'inter_token_latency_p90'
...
def plot_itl_ratio(
    ...
    title: str = "ITL p90 Ratio vs Concurrency",
    ...
    """
    # 计算倍数关系（大值/小值）
    concurrencies, ratios, ratio_labels = calculate_ratio(data_agg, data_disagg, itl_col)
    ...
    if disagg_larger_count >= agg_larger_count:
        ...
        ylabel_text = f'ITL p90 Ratio ({label_disagg} / {label_agg})'
    else:
        # 大多数情况下 agg 更大
        legend_label = f'{label_agg} / {label_disagg}'
        ylabel_text = f'ITL p90 Ratio ({label_agg} / {label_disagg})'
    
    # 绘制倍数曲线（使用正常坐标，不使用对数坐标）
    plt.plot(concurrencies, ratios, 's-', linewidth=2, markersize=8, 
             color='#2ca02c', label=legend_label)
...
def main():
    parser = argparse.ArgumentParser(
        description='Plot ITL p90 ratio (disagg/agg) vs concurrency'
    )
    parser.add_argument(
        '--csv-agg',
        type=str,
    ...
    parser.add_argument(
        ...
        help='ITL column name (default: inter_token_latency_p90)'
    )
    parser.add_argument(
        '--title',
        type=str,
        default='ITL p90 Ratio vs Concurrency',
        help='Plot title'
    )
    ...
    # 检查必需的列
    if args.itl_col not in data_agg[0]:
        print(f"❌ Error: Column '{args.itl_col}' not found in agg CSV")
        return
    if args.itl_col not in data_disagg[0]:
        print(f"❌ Error: Column '{args.itl_col}' not found in disagg CSV")
        return
