...
RESULTS_DIR="$PROJECT_ROOT/cjworkspace/results/sglangv2"
...
# 注意：已经在上面切换到项目根目录了
echo "📈 [1/10] Generating TTFT (p90) vs Concurrency comparison plot..."
...
echo "📈 [2/10] Generating TTFT (avg) vs Concurrency comparison plot..."
python3 "$PLOT_DIR/plot_ttft_concurrency.py" \
    --csv-agg "$AGG_CSV" \
    --csv-disagg "$DISAGG_CSV" \
...
echo "📈 [3/10] Generating ITL (p90) vs Concurrency comparison plot..."
...
echo "📈 [4/10] Generating ITL (avg) vs Concurrency comparison plot..."
python3 "$PLOT_DIR/plot_itl_concurrency.py" \
    --csv-agg "$AGG_CSV" \
    --csv-disagg "$DISAGG_CSV" \
...
echo "📈 [5/10] Generating Concurrency vs Throughput comparison plot..."
...
echo "📈 [6/10] Generating ITL vs Throughput comparison plot..."
...
echo "📈 [7/10] Generating SLO Scaling (Throughput) comparison plot..."
...
echo "📈 [8/10] Generating SLO Scaling (Concurrency) comparison plot..."
python3 "$PLOT_DIR/plot_slo_scaling_throughput.py" \
    --csv-agg "$AGG_CSV" \
    --csv-disagg "$DISAGG_CSV" \
...
# 9. TTFT p90 倍数关系图
echo "📈 [9/10] Generating TTFT p90 ratio comparison plot..."
python3 "$PLOT_DIR/plot_ttft_ratio.py" \
    --csv-agg "$AGG_CSV" \
    --csv-disagg "$DISAGG_CSV" \
    --output "$OUTPUT_DIR/plot_ttft_p90_ratio_compare.png" \
    --ttft-col time_to_first_token_p90
echo "✅ TTFT p90 ratio plot saved"
echo ""

# 10. ITL p90 倍数关系图
echo "📈 [10/10] Generating ITL p90 ratio comparison plot..."
python3 "$PLOT_DIR/plot_itl_ratio.py" \
    --csv-agg "$AGG_CSV" \
    --csv-disagg "$DISAGG_CSV" \
    --output "$OUTPUT_DIR/plot_itl_p90_ratio_compare.png" \
    --itl-col inter_token_latency_p90
echo "✅ ITL p90 ratio plot saved"
echo ""
