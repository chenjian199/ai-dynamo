...
def find_result_files(base_dir: str = None, mode: str = "agg", filter_isl: float = None, filter_osl: float = None) -> Dict[tuple, tuple]:
    """
    查找所有并发度下的 profile_export_genai_perf.json 文件，按 (concurrency, isl, osl, deployment_name) 组合区分
    
    Args:
        base_dir: 基础目录（如果为None，则使用 benchmarks/temp）
    ...
    # 如果未指定base_dir，使用项目根目录下的benchmarks/temp
    if base_dir is None:
        script_dir = Path(__file__).parent
        project_root = script_dir.parent.parent
        base_dir = str(project_root / "benchmarks" / "temp")
    ...
    for test_dir in test_dirs:
        ...
        match = re.match(rf'{mode}_([^_]+)_isl(\d+(?:\.\d+)?)_osl(\d+(?:\.\d+)?)_concurrency(\d+)', dir_name)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Base directory to search for test results (default: benchmarks/temp relative to project root)'
    )
    parser.add_argument(
        '--output-csv',
        type=str,
    ...
    result_files = find_result_files(args.base_dir, args.mode, args.filter_isl, args.filter_osl)
    ...
    if not result_files:
        ...
        if args.base_dir:
            ...
            script_dir = Path(__file__).parent
            project_root = script_dir.parent.parent
            search_dir = str(project_root / "benchmarks" / "temp")
