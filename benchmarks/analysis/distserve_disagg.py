...
class DistServeStyleTest:
    ...
    def __init__(self, service_url: str = None, model_name: str = None, deployment_name: str = None):
        ...
        self.service_url = service_url or os.environ.get('SERVICE_URL', 'http://127.0.0.1:8005')
        self.model_name = model_name or os.environ.get('DEPLOYMENT_MODEL_ID', '/raid5/models/deepseek-ai/DeepSeek-R1-Distill-Llama-70B')
        self.deployment_name = deployment_name or os.environ.get('DEPLOYMENT_NAME', 'disagg')
        
        self.slo_configs = {
            # 基于实际数据分析的SLO配置
            'ultra_strict': {'ttft': 150000, 'tpot': 100},      # 超严格SLO (P50水平)
        }
    ...
    def run_benchmark_with_slo(self, concurrency: int, slo_config: str) -> Dict:
        ...
        # 使用 benchmarks/temp 目录，包含部署名和ISL/OSL信息
        isl = 5000
        osl = 100
        stddev = 0
        ...
        temp_dir = project_root / "benchmarks" / "temp"
        
        # 目录格式: disagg_{deployment_name}_isl{isl}_osl{osl}_concurrency{concurrency}
        output_dir = temp_dir / f"disagg_{self.deployment_name}_isl{isl}_osl{osl}_concurrency{concurrency}"
        ...
        # 从输出目录读取结果 - 使用不同的路径以区分两个测试
        # output_dir 已在上面定义，这里直接使用
        
        # 优先使用完整模型路径的目录
        # 去掉开头的斜杠，然后替换斜杠为下划线，避免生成双下划线
        model_safe_name = self.model_name.lstrip('/').replace('/', '_')  # Only replace slashes, keep hyphens
        result_file = Path(output_dir) / f"_{model_safe_name}-openai-chat-concurrency{concurrency}" / "profile_export_genai_perf.json"
        
        # 如果期望的路径不存在，尝试在 disagg 输出目录中查找
        # 只查找 disagg 自己的路径，不混读 agg 的结果
        if not result_file.exists():
            import glob
            # 在当前输出目录中查找（新格式：benchmarks/temp/disagg_isl*_osl*_concurrency*）
            ...
            if matching_files:
                ...
                print(f"🔍 Found result file in disagg directory: {result_file}")
            else:
                # 回退到模型名目录（仍然在 disagg 目录中）
                ...
                print(f"⚠️  Using fallback path in disagg directory: {result_file}")
        else:
            print(f"✅ Using primary path in disagg directory: {result_file}")
        
        if not result_file.exists():
            print(f"❌ Result file not found in disagg directory: {result_file}")
    ...
    def run_single_deployment_test(self, deployment_name: str = "vllm-disagg"):
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        deployment_name = os.environ.get('DEPLOYMENT_NAME', 'sglang-disagg')
